...
export async function createSlimpayPixCharge(
  ...
  try {
    console.log("[Slimpay] Criando cobranca PIX:", JSON.stringify(request))
    ...
    if (response.ok && response.status === 201) {
      console.log("[Slimpay] Cobranca criada com sucesso:", data)
      return { success: true, data }
    }

    console.error("[Slimpay] Erro ao criar cobranca:", data)
    return {
      success: false,
      error: data.errorCode || "UNKNOWN_ERROR",
      errorCode: data.errorCode,
      message: data.message || "Erro ao criar cobranca PIX",
    }
  } catch (error) {
